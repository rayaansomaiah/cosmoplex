...
function projString_default(defData) {
  ...
  var params2 = {
    ...
    gamma: function(v2) {
      self.rectified_grid_angle = parseFloat(v2) * D2R;
    },
    lonc: function(v2) {
      self.longc = v2 * D2R;
    },
    ...
    towgs84: function(v2) {
      self.datum_params = v2.split(",").map(function(a) {
        return parseFloat(a);
      });
    },
  ...
  if (this.sphere) {
    ...
    if (Math.abs(this.coslat0) <= EPSLN) {
      ...
      if (rh <= EPSLN) {
        ...
        lon = adjust_lon_default(this.long0 + Math.atan2(p.x * Math.sin(ce2), rh * this.cosX0 * Math.cos(ce2) - p.y * this.sinX0 * Math.sin(ce2)), this.over);
      }
...
// node_modules/dotted-map/dist/without-countries.mjs
var PROJECTIONS = {
...
var DEFAULT_PROJECTION = { name: "mercator" };
var getProj4String = (projection = DEFAULT_PROJECTION) => {
  var _a, _b;
  const template = PROJECTIONS[projection.name];
  const lat = ((_a = projection.center) == null ? void 0 : _a.lat) ?? 0;
  const lng = ((_b = projection.center) == null ? void 0 : _b.lng) ?? 0;
...
var DottedMapWithoutCountries = class {
  ...
  getPin({ lat, lng }) {
    const projected = lib_default(this.proj4String, [lng, lat]);
    if (!projected.every((v2) => Number.isFinite(v2))) return;
    ...
    if (y % 2 === 0 && this.grid === "diagonal") rawX -= 0.5;
    let localx = Math.round(rawX);
    const localy = Math.round(y) * this.ystep;
    if (y % 2 === 0 && this.grid === "diagonal") localx += 0.5;
    const inverse34 = lib_default(this.proj4String, "WGS84", [localx * this.X_RANGE / this.width + this.X_MIN, this.Y_MAX - localy * this.Y_RANGE / this.height]);
    if (!inverse34.every((v2) => Number.isFinite(v2))) return;
    const [localLng, localLat] = inverse34;
    return {
      x: localx,
      y: localy,
      lat: localLat,
...
// node_modules/dotted-map/dist/index.mjs
var countries_geo_default = {
...
var DEFAULT_PROJECTION2 = { name: "mercator" };
var getProj4String2 = (projection = DEFAULT_PROJECTION2) => {
  var _a, _b;
  const template = PROJECTIONS2[projection.name];
  const lat = ((_a = projection.center) == null ? void 0 : _a.lat) ?? 0;
  const lng = ((_b = projection.center) == null ? void 0 : _b.lng) ?? 0;
  return template.replace("{lat}", String(lat)).replace("{lng}", String(lng));
};
var isFinitePoint = (point) => point.every((v2) => Number.isFinite(v2));
var typedWorld = countries_geo_default;
var geojsonByCountry = typedWorld.features.reduce((countries, feature) => {
  countries[feature.id] = feature;
  return countries;
}, {});
var geojsonToMultiPolygons = (geojson) => {
  return {
    type: "Feature",
    properties: {},
    geometry: {
      type: "MultiPolygon",
      coordinates: geojson.features.reduce((poly, feature) => poly.concat(feature.geometry.type === "Polygon" ? [feature.geometry.coordinates] : feature.geometry.coordinates), [])
    }
  };
};
var CACHE = {};
...
var computeGeojsonBox = (geojson) => {
  ...
  } else if (geojson.type === "Feature") return computeGeojsonBox(geojson.geometry);
  else if (geojson.type === "MultiPolygon") return computeGeojsonBox({
    type: "Polygon",
...
var getMap = ({ height = 0, width = 0, countries = [], region, grid = "vertical", projection = DEFAULT_PROJECTION2 }) => {
  if (height <= 0 && width <= 0) throw new Error("height or width is required");
  const proj4String = getProj4String2(projection);
  ...
  for (let i = 0; i <= SAMPLES; i++) {
    ...
    for (const point of candidates) {
      const [px, py] = lib_default(proj4String, point);
  ...
  else if (height <= 0) height = Math.round(width * Y_RANGE / X_RANGE);
  const points = {};
  ...
  for (let y = 0; y * ystep < height; y += 1) for (let x = 0; x < width; x += 1) {
    ...
    const wgs84Point = lib_default(proj4String, "WGS84", [localx / width * X_RANGE + X_MIN, Y_MAX - localy / height * Y_RANGE]);
    if (!isFinitePoint(wgs84Point) || !index_default(wgs84Point, poly)) continue;
    points[[x, y].join(";")] = {
      x: localx,
      y: localy
    };
...
var getMapJSON = (props) => JSON.stringify(getMap(props));
var getCacheKey = ({ height = 0, width = 0, countries = [], region, grid = "vertical", projection = DEFAULT_PROJECTION2 }) => {
  return [
...
var DottedMap = class extends DottedMapWithoutCountries {
  constructor({ avoidOuterPins = false, ...args }) {
...
export {
  DottedMapWithoutCountries,
  DottedMap as default,
  getMapJSON
};
//# sourceMappingURL=dotted-map.js.map
